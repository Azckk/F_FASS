...
export function useHook(tableRef: Ref) {
  ...
  function handleUser(rows = selection.value) {
    if (rows.length === 0) {
      message("请至少选择一项数据再进行操作！", { type: "warning" });
      return;
    }
    ...
    addDialog({
      ...
      fullscreenIcon: true,
      closeOnClickModal: false,
      contentRenderer: () => userIndex,
      closeCallBack: () => handleSearch()
    });
  }

  function handlePermission(rows = selection.value) {
    if (rows.length === 0) {
      message("请至少选择一项数据再进行操作！", { type: "warning" });
      return;
    }
    ...
    addDialog({
      ...
      fullscreenIcon: true,
      closeOnClickModal: false,
      contentRenderer: () => h(permissionIndex, { ref: permissionRef }),
      closeCallBack: () => handleSearch(),
      beforeSure: (done, { options, index }) => {
        permissionRef.value.Update();
        message("操作成功！", { type: "success" });
        done();
      }
    });
