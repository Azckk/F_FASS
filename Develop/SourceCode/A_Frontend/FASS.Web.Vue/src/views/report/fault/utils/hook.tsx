...
export function useHook(tableRef?: Ref | undefined) {
  ...
  async function handleSearch() {
    ...
    dataList.value.totalData.forEach(data => {
      let item = modeList.value.find(mode => mode.code === data.code);
      if (item) {
        item.value = data.alarmCount;
      }
    });
    dataList.value.totalData = modeList.value;
    // dataList.value.columnarChartData = [
    //   {
    //     alarmCount: 20,
    //     name: "丢定位报警",
    //     code: "101"
