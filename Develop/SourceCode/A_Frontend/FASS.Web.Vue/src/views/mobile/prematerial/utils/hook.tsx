...
import { getPage, deleteApi } from "@/api/mobile/prematerial";
...
export function useHook(tableRef?: Ref) {
  ...
  const columns: TableColumnList = [
    ...
    {
      headerRenderer: () => t("table.storageName"),
      label: "table.storageName",
      prop: "storageName",
      sortable: true
    },

    {
      headerRenderer: () => t("table.isScheduled"),
      label: "table.isLock",
      prop: "isLock",
      sortable: true,
      cellRenderer: ({ row }) => (
        <div style={{ color: row.isLock ? "red" : "green" }}>
          {row.isLock ? "被预定" : "未预定"}
        </div>
      )
      // formatter: ({ isLock }) => {
