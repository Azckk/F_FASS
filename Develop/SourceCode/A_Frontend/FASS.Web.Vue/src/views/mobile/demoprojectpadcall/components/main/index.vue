...
import { AddWork3 } from "@/api/mobile/padcall";
...
watchEffect(() => {
  ...
  if (dataInfo.value && dataInfo.value.length > 0) {
    if (dataInfo.value[0].areaCode == "FYBF") {
      isFlag = true;
    } else {
      isFlag = false;
    }
...
async function sendWork(e, callMode, type) {
  ...
  let res = await AddWork3(data.storageId, data.type);
  if (res.code === 200) {
    ElMessage({
      message: "呼叫成功",
      type: "success"
      ...
      <div class="imgBox">
        <div v-if="item.type === 'Put'" style="width: 100%">
          <div style="display: flex; height: 100%">
            <div
              :class="isFlag ? 'discharged' : 'dischargeds'"
            ...
            <div v-if="isFlag" class="discharged" style="position: relative">
