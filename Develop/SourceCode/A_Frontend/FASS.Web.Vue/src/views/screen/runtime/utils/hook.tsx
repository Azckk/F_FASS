import { ref, onMounted, watch, onUnmounted, reactive, watchEffect } from "vue";
...
export function useHook() {
  ...
  let lockNodeLength = 0;
  // 自动站点禁用样式
  const changeNodesState = lockNode => {
    if (lockNode.length == lockNodeLength) return;
    if (lockNode && lockNode.length) {
      ...
      lockNodeLength = lockNode.length;
    }
  };
  // 手动禁用样式
