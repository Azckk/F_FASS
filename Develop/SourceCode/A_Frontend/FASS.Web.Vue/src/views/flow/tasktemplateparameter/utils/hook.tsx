...
import {
  ...
  putTaskTemplateParameter,
  DeleteTaskTemplateParameter,
  getTaskTemplateParameterPage
...
export function useHook(
  ...
  async function handleSearch() {
    ...
    const { data } = await getTaskTemplateParameterPage(
      keyValue.value,
      pageParam.value
    );
    ...
    addDialog({
      ...
      beforeSure: (done, { options, index }) => {
        ...
        formRef.value.getRef().validate(async valid => {
          if (valid) {
            await putTaskTemplateParameter("", formData);
  ...
  function handleUpdate(rows = selection.value) {
    ...
    addDialog({
      ...
      beforeSure: (done, { options, index }) => {
        ...
        formRef.value.getRef().validate(async valid => {
          if (valid) {
            await putTaskTemplateParameter(row?.id, formData);
            message("操作成功！", { type: "success" });
            handleSearch();
            done();
          }
  ...
  async function handleDelete(rows = selection.value, cancel = undefined) {
    ...
    ElMessageBox.confirm("是否确认操作？", "提示", {
      ...
      .then(async () => {
        await DeleteTaskTemplateParameter(rows.map(e => e.id));
        message("操作成功！", { type: "success" });
        handleSearch();
      })
      .catch(() => cancel?.());
