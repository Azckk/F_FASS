using Common.Frame.Contexts;
using Common.Service.Pager;
using Common.Service.Pager.Models;
using Common.Service.Services.Interfaces;
using FASS.Service.Dtos.FlowExtend;
using FASS.Service.Entities.FlowExtend;

namespace FASS.Service.Services.FlowExtend.Interfaces
{
    public interface ITaskTemplateRuleService : IAuditService<FrameContext, TaskTemplateRuleEntity, TaskTemplateRuleDto>
    {
        Task<IPage<TaskTemplateRuleDto>> ToPageAsync(string taskTemplateId, Page page);
    }
}
