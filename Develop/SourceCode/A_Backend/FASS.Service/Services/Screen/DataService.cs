...
namespace FASS.Service.Services.Screen
{
    public class DataService : AuditService<FrameContext, AuditEntity, AuditDto>, IDataService
    {
        ...
        public IEnumerable<dynamic> getData1()
        {
            var data = Repository.GetDbConnection().Select(@"
                SELECT '涂布+' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '涂布-' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '辊压+' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '辊压-' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '模切+' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '模切-' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '卷绕+' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '卷绕-' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value");
            return data;
        }

        public dynamic getData2()
        {
            var data = Repository.GetDbConnection().Fetch(@"
                SELECT 
                CAST(FLOOR(RANDOM()*100) AS INT) AS count1,
                CAST(FLOOR(RANDOM()*100) AS INT) AS count2,
                CAST(FLOOR(RANDOM()*100) AS INT) AS count3,
                CAST(FLOOR(RANDOM()*100) AS INT) AS count4,
                CAST(FLOOR(RANDOM()*100) AS INT) AS count5,
                CAST(FLOOR(RANDOM()*100) AS INT) AS count6,
                CAST(FLOOR(RANDOM()*100) AS INT) AS count7,
                CAST(FLOOR(RANDOM()*100) AS INT) AS count8");
            return data;
        }

        public IEnumerable<dynamic> getData3()
        {
            var data = Repository.GetDbConnection().Select(@"
                SELECT '涂布+' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
        ...
        public IEnumerable<dynamic> getData4()
        {
            var data = Repository.GetDbConnection().Select(@"
                SELECT '未运行' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '运行中' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '停止中' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '充电中' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value");
            return data;
        }

        public IEnumerable<dynamic> getData5()
        {
            var data = Repository.GetDbConnection().Select(@"
                SELECT '1' AS carNo,'涂布+' AS area,'急停' AS alarm,'2022/04/12 12:25:15' AS datetime
                UNION ALL
                SELECT '2' AS carNo,'涂布-' AS area,'网络异常' AS alarm,'2022/04/12 12:25:10' AS datetime
                UNION ALL
                SELECT '3' AS carNo,'辊压+' AS area,'障碍物异常报警' AS alarm,'2022/04/12 12:25:05' AS datetime
                UNION ALL
                SELECT '4' AS carNo,'辊压-' AS area,'安全触边报警' AS alarm,'2022/04/12 12:24:55' AS datetime
                UNION ALL
                SELECT '5' AS carNo,'模切+' AS area,'驱动报警' AS alarm,'2022/04/12 12:24:45' AS datetime
                UNION ALL
                SELECT '6' AS carNo,'模切-' AS area,'低电压报警' AS alarm,'2022/04/12 12:24:35' AS datetime
                UNION ALL
                SELECT '7' AS carNo,'卷绕+' AS area,'低电量报警' AS alarm,'2022/04/12 12:24:25' AS datetime
                UNION ALL
                SELECT '8' AS carNo,'卷绕-' AS area,'急停' AS alarm,'2022/04/12 12:24:25' AS datetime
                UNION ALL
                SELECT '9' AS carNo,'涂布+' AS area,'网络异常' AS alarm,'2022/04/12 12:24:15' AS datetime
                UNION ALL
                SELECT '10' AS carNo,'涂布-' AS area,'障碍物异常报警' AS alarm,'2022/04/12 12:23:55' AS datetime");
            return data;
        }

        public IEnumerable<dynamic> getData6()
        {
            var data = Repository.GetDbConnection().Select(@"
                SELECT '急停' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '网络异常' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '障碍物异常报警' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '安全触边报警' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '驱动报警' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '低电压报警' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value
                UNION ALL
                SELECT '低电量报警' AS name,CAST(FLOOR(RANDOM()*100) AS INT) AS value");
            return data;
        }
        ...
        public async Task<(dynamic,dynamic)> GetCarStateAsync()
        {
            ...
            {
                name = "在线",
                value = cars.Where(e => e.IsOnline && e.IsNormal).Count()
            };
            return (list, onlineCar);
        }

        public async Task<(dynamic, dynamic)> GetChargeStateAsync()
        {
            ...
            var onlineCharge = new
            {
                name = "在线",
                value = charges.Where(e => e.IsEnable &&  e.State == ChargingStationConst.State.Online).Count()
            };
            return (list, onlineCharge);
        }
    }
}